...
Test Data Coverage は Business Coverage と別の実行時 Gate である。確定した AcceptanceCriteria、TestCase、`test_data_id` の全組合せを母数とし、TestDataPlan の `coverage_conditions` が不足、範囲外、重複、Identity Key だけの存在確認、未確認 Observation 参照であれば受理しない。実行時は各 `test_data_id` を登録済みの実 `DataIdentityProvider`（database／api／ui／hybrid）へ結び、実観測から `primary_key`、`business_unique_keys`、`screen_identity_values`、`record_scope_locator`、`match_count=1`、`evidence_ref` を同一形式で固定する。Hybrid は各 Source の同名・同値 Identity による連結を必須とし、全 Source が同一業務レコードを観測したことを検証する。未登録 Provider、必要な source Evidence の欠落、fake、推測、静かな fallback は blocked とする。業務項目、状態、境界値、関連関係の Coverage は同じ Provider の確認済み実 Observation から評価し、database は SQL readback、api は HTTP response、ui は Playwright Observation、hybrid は条件ごとに明示した実 Source を使用する。期待値、実測値、Observation Source、判定、digest、`data_coverage` Evidence を Run に固定し、エンジン算出値が 100% になる前に TestPlan の UI Step を開始しない。Coverage の分母、実測値、成功判定を Copilot の自己申告から取得しない。
...
既存データは Project の確認済み `DataIdentityProvider` を実行して一意候補を作り、候補をその Test Case の Change Request に固定する。人工確認は Provider 行を同一 Transaction で再 Lock／照合し、成功時には Change Request 専用の完全な Plan Revision Task を同じ Transaction で発行する。新しい UI TestPlan／TestDataPlan v3 が Schema、安全性、Business Coverage 100%、Test Data Coverage 条件の静的対応と人工確認を再通過した後だけ正式な `binding_mode=adopted` 計画となる。実 Run は実 Observation から Test Data Coverage を計算し、100% の後だけ UI Step を開始する。別 Change Request の登録や旧 Plan は現在実行へ影響しない。Run 中に解決した `TestDataBinding` は `project_id`、`run_id`、`test_data_id`、identity／content digest で固定し、同一 Run の後続 Flow と別 Test Caseから参照できる。一度凍結した Binding の更新・上書き、別 Run／別 Project の参照、digest 不一致は禁止する。
