...
- TestDataPlan の各 `test_data_id` は確認済み SQL readback から実 DB 主キー、業務一意キー、画面識別キーを 1 件だけ取得し、Run 固有 Binding／digest／Evidence として固定する。全 UI Step は `operation_scope=screen|bound_record` を明示し、跨画面・表 UI Step は `bound_record` と `data_binding_ref` と exact Locator でこの 1 件だけを操作する。分類欠落・矛盾、0 件、複数件、drift、行番号、曖昧 Text、AI 推測は停止理由となり、固定済み Scope に computer-use fallback は使わない。
