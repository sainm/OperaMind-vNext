...
- Copilot は全 AcceptanceCriteria／TestCase／TestData の組合せに `coverage_conditions` を出力するが、Coverage 値や成功判定は出力しない。OperaMind が同じ DataIdentityProvider の確認済み実 Observation（database SQL readback／API response／Playwright Observation／hybrid の明示 Source）から項目、状態、境界値、関連関係を検証し、期待値／実測値／Observation Source／判定を Evidence に固定する。Test Data Coverage が 100% になる前に TestPlan の UI Step を開始しない。
