...
`0036` 增加生成 UI Test Case 的自然语言修订台账。`TestCaseChangeProposal` 固定原始 Orchestration、原始 UiTestPlan、自然语言和跨多个 Case 的结构化操作；确定性操作也必须先展示整体差异，有歧义时在同一 Proposal 中收集全部明确选项，无法安全解释时整体阻断，不做部分修改。统一确认后只创建一个 read-only `ui_test_plan_revision` Copilot Task；Copilot 返回完整的 UiTestPlan、跨画面 TestDataPlan 和 Playwright 步骤并通过门禁后，才在同一事务中创建新的 AcceptanceCriteria、TestPlan、TestDataPlan、BusinessCoverageReport 与 ChangeOrchestrationPlan。原 Orchestration 通过显式 supersession 边转为 `superseded`；旧 Run、执行 Artifact、Evidence、Screenshot 和 Closure 的引用写入 stale 清单，读取当前状态时不得跨 Orchestration 复用。由于修订版本共享同一已确认设计、Impact Report 和 Copilot Planning Source 摘要，`0031` 的 basis 级唯一约束在本 migration 中移除，版本唯一性改由不可变 ID、单 Proposal 单 Revision 和 supersession 状态保证。
