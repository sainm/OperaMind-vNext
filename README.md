...
Golden Dataset、実 Embedding Provider、人手 Approval、target Deployment E2E には既存の通過 Evidence があります。現在の有効状態は Manifest の記載値ではなく Evidence 検証後の CLI／API 出力を正とし、source tree が変わった旧 `full_local_regression` は `pending`、Manifest は `stale` と扱います。実際の VS Code GitHub Copilot 完了 receipt も未完了です。Golden RAG はコードと PostgreSQL 統合回帰が完成していますが、運用上の通過状態は Canonical PostgreSQL にある同一 Scope の最新 `GoldenRagQualityReport` だけで判定します。この作業環境には実 VisionDemo Snapshot／Index に対する監査可能な Report がないため、過去の対話メモの数値を正式通過とは扱いません。品質ゲートは [再現可能な品質ベースライン](docs/QUALITY-BASELINE.md)、未完了条件は [後続タスクリスト](docs/NEXT-TASKS.md) を参照してください。
