    ...
    def authorize_ui(
        ...
        *,
        ...
        if lock:
            self.lock(grant_id=grant_id, project_id=project_id)
        grant = self.inspect(grant_id, at=at)
        if (grant.project_id, grant.edit_packet_id) != (project_id, edit_packet_id):
            raise ValueError("Approval Grant does not match UI Execution scope")
        if grant.state not in {"ui_pending", "completed"}:
            raise ValueError(f"Approval Grant does not permit UI execution in state: {grant.state}")
        if "execute_ui" not in grant.allowed_actions:
            raise ValueError("Approval Grant does not allow UI execution")
        if tuple(sorted(scenario_refs)) != tuple(sorted(grant.allowed_ui_scenarios)):
            raise ValueError("Approval Grant UI Scenario scope differs from Execution Plan")
        self._assert_ui_source_current(grant, lock=lock)
        return grant

    def _assert_ui_source_current(
        self,
        grant: ApprovalGrantAuthorization,
        *,
        lock: bool,
        ...
        locking = (
            "FOR UPDATE OF packet, report, case_record, result FOR SHARE OF graph" if lock else ""
        )
        with self._connection.cursor() as cursor:
            cursor.execute(
                f"""
                SELECT packet.status, report.status, graph.is_current, graph.status,
                       case_record.status, result.validation_mode, result.status,
                       result.tests_passed, result.command_evidence_status,
                       packet.impact_report_id, packet.confirmation_id,
                       packet.repository_id, packet.base_repository_revision,
                       report.repository_revision, result.base_repository_revision
                FROM edit_packets AS packet
                JOIN impact_reports AS report
                  ON report.impact_report_id = packet.impact_report_id
                 AND report.project_id = packet.project_id
                ...
                JOIN edit_results AS result
                 ON result.edit_packet_id = packet.edit_packet_id
                 AND result.project_id = packet.project_id
                 AND result.approval_grant_id = %s
                 AND result.validation_mode = 'committed'
                WHERE packet.edit_packet_id = %s
                  AND packet.project_id = %s
                {locking}
                """,
                (grant.grant_id, grant.edit_packet_id, grant.project_id),
            )
            source = cursor.fetchone()
        ...
        if source is None or duplicate is not None or tuple(source) != expected:
            raise ValueError("Approval Grant source is no longer current for UI verification")

    def authorize_ui_plan(
        self,
        *,
        grant_id: str,
        project_id: str,
        ...
        with self._connection.cursor() as cursor:
            cursor.execute(
                f"""
                SELECT edit_packet_id, scenario_refs
                FROM ui_execution_plans
            ...
            row = cursor.fetchone()
        if row is None:
            raise ValueError("Approval Grant UI Execution Plan does not exist")
        return self.authorize_ui(
            grant_id=grant_id,
            project_id=project_id,
            edit_packet_id=str(row[0]),
            scenario_refs=_strings(row[1]),
            at=at,
            lock=lock,
        )
