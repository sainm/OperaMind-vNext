...
class MainFlowCoordinator:
    ...
    def run_once(self) -> MainFlowCoordinatorIteration:
        ...
        with psycopg.connect(self.database_url, autocommit=True) as connection:
            ...
            candidates = ChangeAutomationRepository(connection).list_coordinator_candidates(
                limit=self.candidate_limit
            )
            for candidate in candidates:
                request_id = candidate.change_request_id
                observed += 1
                try:
                    ...
                    elif _test_data_execution_is_running(automation):
...
def _test_data_execution_is_ready(automation: dict[str, object] | None) -> bool:
    return CHANGE_FLOW_STATE_MACHINE.is_ready_for_action(
        automation,
...
def _ui_publication_is_ready(automation: dict[str, object] | None) -> bool:
    return CHANGE_FLOW_STATE_MACHINE.is_ready_for_action(
        automation,
        action="run_ui_verification",
    )


def _test_data_execution_is_running(automation: dict[str, object] | None) -> bool:
    return CHANGE_FLOW_STATE_MACHINE.is_running_action(
        automation,
        action="refresh",
    )
