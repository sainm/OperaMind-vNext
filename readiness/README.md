...
`mvp-readiness.silver.json` records the original development baseline. `mvp-readiness.json` stores declared gate state, while `MvpReadinessValidator` output is the only effective repository-readiness state. The selected VisionDemo case is frozen and `golden_dataset` has passed with reviewed evidence. The real local Nomic Provider contract, VisionDemo v3 human Impact/Grant chain, and revision-bound target Deployment have evidence. GitHub Copilot remains pending. A PostgreSQL/Edge full-regression report becomes historical as soon as its source digest is stale; status output then reports the affected gate as `pending` and the manifest as `stale`, even if the persisted manifest still says `passed`.
