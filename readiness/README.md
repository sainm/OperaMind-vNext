...
`mvp-readiness.silver.json` records the original development baseline. `mvp-readiness.json` stores declared gate state, while `MvpReadinessValidator` output is the only effective repository-readiness state. The selected VisionDemo case is frozen and `golden_dataset` has passed with reviewed evidence. The real local Nomic Provider contract and VisionDemo v3 human Impact/Grant chain also have evidence. GitHub Copilot, current target Deployment, and full local regression remain pending until evidence is regenerated against the reconstructed flow and final source tree. A PostgreSQL/Edge full-regression report becomes historical as soon as its source digest is stale; status output then reports the affected gate as `pending` and the manifest as `stale`, even if the persisted manifest still says `passed`.
...
Scope authorization and Deployment evidence require no external receipt: authorization is derived from the internal normalized Impact/Confirmation/Packet/Grant chain, while Deployment evidence is derived from the current Orchestration, its latest TestData execution, bounded UI result, and Closure state. Those controls are not separate user operations. Copilot is intentionally different because edit rows alone do not prove the editor origin. Record a reviewed session only after VS Code GitHub Copilot completed the immutable unified Change Task through the five public MCP tools.
...
Do not infer the current result from a historical count in this document. Run `operamind-baseline --print-readiness-json`: a digest mismatch makes `manifest_status=stale`, exposes the validation issue, and makes `full_local_regression` effectively pending. The gate may return to passed only after the fixed suite completes with PostgreSQL and real Microsoft Edge, with zero failures and zero skips, and the generated Evidence validates against the final tree. Do not change an external gate to `passed` merely because implementation code or a Fake-based test exists.
...
- `target_deployment_e2e`: `readiness/templates/target-deployment-e2e.example.json`; fill it only after the current Orchestration's TestDataPlan UI steps pass against the revision-bound target and sanitized screenshot evidence has stable IDs.
